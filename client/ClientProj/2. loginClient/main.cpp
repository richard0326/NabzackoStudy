...
int main(int argc, char* argv[])
{
	...
	while (true)
	{
		...
		LoginInfo login;
		printf("아이디 : ");
		if (fgets(login.ID, sizeof(char) * 20, stdin) == NULL)	// 입력을 받고 입력 실패시 탈출
			break;

		printf("비밀번호 : ");
		if (fgets(login.PW, sizeof(char) * 20, stdin) == NULL)	// 입력을 받고 입력 실패시 탈출
			break;

		// 데이터 보내기
		memcpy(buf, &login, sizeof(LoginInfo));
		retval = send(sock, buf, sizeof(LoginInfo), 0);	// 서버로 데이터를 보낸다.
